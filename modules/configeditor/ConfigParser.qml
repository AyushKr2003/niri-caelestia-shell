...
Singleton {
    ...
    readonly property var configSections: [
        ...
        { name: "paths", title: "Paths", icon: "folder" }
    ]
    ...
    FileView {
        ...
        onLoaded: {
            try {
                ...
                console.error("ConfigParser: Failed to parse shell.json:", e);
        ...
        onFileChanged: {
            ...
            try {
                ...
                console.error("ConfigParser: Failed to parse shell.json after change:", e);
            }
        }

        onLoadFailed: err => {
            console.warn("ConfigParser: Failed to load shell.json:", FileViewError.toString(err));
    ...
    Process {
        ...
        onExited: (code, status) => {
            if (code !== 0) {
                console.error("ConfigParser: Failed to save config");
            }
    ...
    function saveConfig() {
        ...
        try {
            ...
            console.error("ConfigParser: Failed to save config:", e);
            root.isSaving = false;
            return false;
        }
    }
    ...
    function updateValue(path: list<string>, value): void {
        ...
        for (let i = 0; i < path.length - 1; i++) {
            if (!obj[path[i]]) {
                console.warn("ConfigParser: Path segment not found in configData:", path[i], "at index", i);
                return;
            }
            obj = obj[path[i]];
        }
