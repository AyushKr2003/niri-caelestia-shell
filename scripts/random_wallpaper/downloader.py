    ...
    try:
        with requests.get(url, headers=HEADERS, stream=True, timeout=30) as resp:
            ...
            if len(data) < MIN_VALID_SIZE_BYTES:
                print(f"         ✗ Too small ({len(data)/1024:.1f} KB) — likely error page")
                return None

            with open(save_path, "wb") as f:
                f.write(data)

            print(f"         ✓ {len(data)/1024:.0f} KB saved → {save_path}")
            return save_path
...
def download_best_wallpaper(
    ...
    print("\n   ⚠  All full-resolution attempts failed.")
    print("   → Falling back to thumbnail (low resolution preview only)...")
    ...
    if thumb_url:
        ...
        if result:
            print("   ⚠  Only thumbnail was available for this wallpaper.")
            return result

    print("[ERROR] All download attempts failed for this wallpaper.")
    return None